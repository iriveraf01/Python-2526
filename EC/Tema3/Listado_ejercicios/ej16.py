...
# y añadir a la clase Movie el método __lt__ (less than) y en este programar cuando
...
class Movie:
    ...
    def __str__(self):
        return f"Título: {self.titulo}, Duración: {self.minutos} min, Productora: {self.productora}"
...
class GestionMovie:
    list_movies = []
    @classmethod
    def pelis_por_productora(cls, productora):
        pelis = []
        for movie in cls.list_movies:
            if movie.productora == productora:
                pelis.append(movie)
        return pelis

    @classmethod
    def pelis_con_cadena_en_titulo(cls, cadena):
        pelis = []
        for movie in cls.list_movies:
            if cadena.lower() in movie.titulo.lower():
                pelis.append(movie)
        return pelis
    
    @classmethod
    def numero_de_pelis_mayores_a_la_media(cls):
        total_duracion = 0
        for movie in cls.list_movies:
            total_duracion += movie.minutos
        duracion_media = total_duracion / len (cls.list_movies)
        contador = 0
        for movie in cls.list_movies:
            if movie.minutos > duracion_media:
                contador += 1
        return contador

    @classmethod
    def ordenar_por_duracion(cls):
        cls.list_movies.sort()
...
for movie in list_movies:
    GestionMovie.list_movies.append(movie)

# print("###\t###\t###\t###")
# warner_bros = GestionMovie.pelis_por_productora("Warner Bros. Pictures")
# for peli in warner_bros:
#     print(peli)
# print("###\t###\t###\t###")
# the = GestionMovie.pelis_con_cadena_en_titulo("the")
# for peli in the:
#     print(peli)
# print("###\t###\t###\t###")
# print(f"El número de pelis con duracion mayor a la media es de {GestionMovie.numero_de_pelis_mayores_a_la_media()}.")
# print("###\t###\t###\t###")
# GestionMovie.ordenar_por_duracion()
# for peli in GestionMovie.list_movies:
#     print(peli)