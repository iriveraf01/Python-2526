...
class Prestamo:
    ...
    def __init__(self, libro:Libro, usuario:str, fecha_prestamo:datetime = None):
        self.libro = libro
        if not libro.es_prestable():
            raise ValueError("El libro no es prestable.")
    ...
    def __str__(self):
        estado = "Devuelto" if self.fecha_devolucion_real != None else "Activo"
        return f"Préstamo [{estado}] - {self.libro} - Usuario: {self.usuario} - Fecha límite: {self.fecha_devolucion_limite}"
...
class Biblioteca:
    ...
    def realizar_prestamo(self, libro:Libro, usuario:str) -> bool:
        try:
            ...
            print(f"Éxito ({nuevo_prestamo.fecha_devolucion_limite})")
            return True
    ...
    def devolver_libro(self, isbn:str, fecha_devolucion:datetime = None) -> dict:
        fecha_devolucion = fecha_devolucion if fecha_devolucion else datetime.now()
        for i in self.prestamos_activos:
            if i.libro.isbn == isbn:
                info = i.devolver_libro(fecha_devolucion)
                self.prestamos_activos.remove(i)
                return info
