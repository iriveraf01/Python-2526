...
import os
class Producto:
    def __init__(self, codigo:str, nombre:str, categoria:str, precio:float, stock:int, fecha_creacion: datetime = None) -> None:
        if precio < 0:
    ...
    def to_dict(self) -> dict:
        return { 
            ...
            'precio': self.precio, 
            'stock': self.stock, 
            'fecha_creacion': self.fecha_creacion.isoformat()  # formato ISO: fecha_creacion.isoformat() 
        }
...
class MovimientoInventario:
    ...
    def to_dict(self) -> dict:
        return { 
            ...
            'fecha': self.fecha.isoformat(),  # formato ISO 
            'observaciones': self.observaciones 
        }
    
    def __str__(self) -> str:
        return f"[{self.fecha}] {self.tipo} - {self.producto_codigo} - {self.cantidad} unidades - {self.observaciones}"
...
class Inventario:
    ...
    def agregar_producto(self, producto:Producto) -> bool:
        if producto.codigo in self.productos:
            print(f"ERROR: este producto ya existe.")
            return False
    ...
    def registrar_entrada(self, codigo:str, cantidad:int, observaciones:str = "") -> bool:
        producto_buscado = self.buscar_producto(codigo)
        if not producto_buscado: 
            print(f"Error: Producto con código '{codigo}' no encontrado.")
            return False
        try:
            producto_buscado.agregar_stock(cantidad)
            movimiento = MovimientoInventario(codigo, "ENTRADA", cantidad, observaciones)
            self.movimientos.append(movimiento)
            print(f"Entrada registrada: {cantidad} unidades de '{producto_buscado.nombre}'.")
            return True
    ...
    def listar_productos(self, categoria: str = None) -> None:
        ...
        for producto in self.productos.values():
            if categoria is None or producto.categoria == categoria:
    ...
    def listar_movimientos(self, codigo_producto: str = None, limite: int = 10) -> None:
        ...
        for movimiento in self.movimientos:
            if codigo_producto is None or movimiento.producto_codigo == codigo_producto:
                movimientos_filtrados.append(movimiento)

        # Ordenar por fecha descendente
        movimientos_filtrados.sort(key=lambda m: m.fecha, reverse=True)

        # Aplicar límite
        movimientos_filtrados = movimientos_filtrados[:limite]
